...
interface ActiveStatsCardsProps {
    attendees: number;
    maxCapacity: number;
    revenue: number;
...
export function ActiveStatsCards({ attendees, maxCapacity, revenue, topZones }: ActiveStatsCardsProps) {
    const { t } = useLanguage();
        ...
        <div className="flex gap-4">
            ...
            <div className="bg-[#1A1A1A] rounded-2xl p-4 flex items-center gap-4 border border-white/5 min-w-[240px]">
                ...
                <div>
                    ...
                    <div className="text-2xl font-black text-white">{attendees.toLocaleString()} <span className="text-sm font-medium text-gray-500">{t('dash.people')}</span></div>
            ...
            <div className="bg-[#1A1A1A] rounded-2xl p-4 border border-white/5 min-w-[200px]">
                ...
                <div className="space-y-1">
                    {topZones.slice(0, 3).map((zone, i) => (
                        <div key={i} className="flex justify-between text-xs gap-4">
                            ...
                            <span className="font-mono text-[#D6F32F]">{Math.round(zone.percentage)}%</span>
                        </div>
                    ))}
                </div>
            </div>
