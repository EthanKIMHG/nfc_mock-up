...
export function ScanVisual({ status, mode, onTrigger }: ScanVisualProps) {
    ...
    return (
        <div className="absolute inset-0 z-0 flex items-center justify-center cursor-pointer" onClick={onTrigger}>
            ...
            >
                {/* Ambient Glow */}
                <div className={`absolute inset-0 rounded-full blur-[80px] transition-colors duration-1000 ${status === "SCANNING" ? "bg-purple-600/20" :
                ...
                {status !== "IDLE" && status !== "SUCCESS" && [1, 2, 3].map((i) => (
                    <motion.div
                        key={i}
                        className={`absolute inset-0 rounded-full border ${status === "SCANNING" ? "border-purple-500/20" :
