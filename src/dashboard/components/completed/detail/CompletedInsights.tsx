...
export function CompletedInsights({ insights }: CompletedInsightsProps) {
    const { t } = useLanguage();
        ...
        <div className="bg-[#1A1A1A] rounded-3xl p-6 border border-white/5">
             ...
             <div className="grid grid-cols-2 gap-4">
                    ...
                    <div key={i} className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex gap-4 items-start">
                         ...
                         <div>
                             <div className="flex justify-between items-start mb-1 gap-4">
                                <h4 className="font-bold text-red-400">{insight.title}</h4>
                     ...
                     <div className="col-span-2 text-center text-gray-500 py-8">No specific insights available</div>
                 )}
                 ...
                 <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 flex gap-4 items-start">
                         ...
                         <div>
                             <div className="flex justify-between items-start mb-1 gap-4">
                                <h4 className="font-bold text-blue-400">{t('dash.insight.revenue_opp')}</h4>
                                <span className="text-[10px] bg-blue-500 text-black px-1.5 py-0.5 rounded font-bold shrink-0">{t('dash.filter_insight')}</span>
                             </div>
                             <p className="text-xs text-gray-400 leading-relaxed">
                                {t('dash.insight.beer_trend')}
                             </p>
                             <p className="text-[10px] text-gray-600 mt-2">15m ago</p>
                         </div>
                    </div>
             </div>
         </div>
